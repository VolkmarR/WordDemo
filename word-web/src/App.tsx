...
type Approach = 'home' | 'a' | 'b' | 'c' | 'd' | 'e'

const TABS: { id: Approach; label: string; desc: string }[] = [
    {id: 'home', label: 'Home', desc: 'Select a DOCX file to preview.'},
    {
        id: 'a',
...
function App() {
    ...
    return (
        <div className="app">
            <div>Current approach: {approach}</div>
            ...
            <main className="viewport">
                ...
                {approach === 'home' && (
                    <Home
                        ...
                        onOpenPreview={() => setApproach('a')}
                    />
                )}
                {approach === 'a' && !backend && (
                    <div>Backend is null</div>
                )}

                {approach === 'a' && backend && (
                    <>
                        <div>Loaded</div>
                        <DocView model={backend.model}/>
                    </>
                )}

                {approach === 'b' && (
                    <Suspense fallback={<div className="msg">Loading docx-preview…</div>}>
                        <DocxPreviewView file={file!}/>
                    </Suspense>
                )}

                {approach === 'c' && (
                    <Suspense fallback={<div className="msg">Loading mammoth…</div>}>
                        <MammothView file={file!}/>
                    </Suspense>
                )}
